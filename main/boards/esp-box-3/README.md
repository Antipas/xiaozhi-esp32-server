...
### 4. ESP32-S3 开发版语音异常，是否需要用 ESP-BOX-3 映射？
如果设备表现为“麦克风杂音、唤醒不稳定、或提示无法连接服务”，且硬件为 ESP32-S3 + ES8311/ES7210 组合，建议优先按 `ESP BOX 3` 板型配置。

建议检查项：
- `sdkconfig` 中应为 `CONFIG_BOARD_TYPE_ESP_BOX_3=y`
- `sdkconfig` 中 OTA 地址建议保持官方默认：`CONFIG_OTA_URL="https://api.tenclass.net/xiaozhi/ota/"`
- 串口日志应出现 `SKU=esp-box-3`
- AFE 日志应出现 `total 2 channels(1 microphone, 1 playback)`（说明 AEC 参考通道已建立）
